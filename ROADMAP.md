...
- [ ] 建立正样本、困难负样本和近似措辞反例。
- [ ] 从数据中生成 `rules-v1`，不凭预设关键词直接定案。
- [ ] 每条规则至少配一个正样本和一个反样本。
- [ ] 输出混淆矩阵、precision、recall、误报列表和漏报列表。
- [ ] 冻结规则版本、schema、测试集哈希和研究报告。
...
- 困难负样本：已回溯并人工复核 28 条已登录时间线样本及其父帖/引用语境；去重后包含 27 条负样本和 1 条与正样本集重复的重置公告，用于校验采集链路。复核集 SHA-256 为 `1098e0dd402d6b95e41470a679aca77e3622ab89f0046ed49ce82ca680d7d9d5`。
- 候选规则：从上述人工数据生成研究区实验规则；62 条去重记录的混淆矩阵为 TP=35、FP=0、FN=0、TN=27，precision=1.0000、recall=1.0000，数据 SHA-256 为 `833a1e8d75c3c1c813d51d898b215e646edbda5b579e802b9eb803d6ca3c2a15`。每条候选规则均记录正样本和反例，报告位于 `research/reports/candidate-rules-v1.md`。
