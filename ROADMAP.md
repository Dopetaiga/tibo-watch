...
阶段证据（2026-08-11，待外部授权项）：19 个测试文件、52 项测试通过；完整模拟链证明候选帖经规则、模拟 AI、confirmed 事件、幂等通知进入大屏统计，普通帖保持零 AI/事件/通知。24 小时加速测试覆盖 429、503、断网、退避上限、重复帖与 10,000 条默认内存窗口。`research:verify` 回放 1,684 条历史记录，TP=58、FP=0、FN=0、TN=1,626。当前源码的 NSIS 与便携版构建成功；打包 E2E、便携启动，以及安装版首次启动→写状态→覆盖升级→恢复→卸载→数据保留均通过。真实 FxTwitter 只读连接测试沿用 2026-08-09 已完成结果，不重复抓取。用户文档、隐私、故障排查、导出、发布清单和 0.1.0 Release Notes 草稿已落盘。尚待用户提供并授权 DeepSeek Key、webhook 目标，以及指定 GitHub 仓库后创建线上草稿 Release。
